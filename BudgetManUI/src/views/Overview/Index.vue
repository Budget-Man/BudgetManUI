<template>
  <el-space class="moneyOverviewContainer" fill direction="vertical" :size="30">
    ...
    <el-row justify="space-around">
      <el-text size="large" class="absolute-left"
    ...
    <el-row justify="space-between">
      <el-col :span="11">
        ...
        >
          <el-table-column
            v-for="column in incomeCol.filter((x) => !x.hidden)"
            :key="column.key"
            :prop="column.key"
            :label="column.label"
            :sortable="false"
  ...
  <MnEditItem
    ...
    :columns="incomeCol"
    apiName="income"
    :openDialog="isOpenCreateIncomeDialog"
    :title="$t('income.name')"
    createUrl=""
    editUrl=""
    :isEdit="false"
    :edit-item="newIncomeItem"
    @onSaved="AddIncome"
    @onCloseClicked="CloseCreateIncome"
  />

  <CreateMoneySpend
    :openDialog="isOpenCreateMoneySpendDialog"
    :item="newMoneySpendItem"
    @onSaved="AddMoneySpend"
    @onCloseClicked="CloseCreateMoneySpend"
  >
  </CreateMoneySpend>
</template>
...
const incomeSummary = computed(() => {
  ...
  return incomeData.value.reduce(
    (sum: number, row: any) => sum + Number(row[lastColumnKey]),
    0
  );
});
const expenseSummary = computed(() => {
  ...
  return expenseData.value.reduce(
    (sum: number, row: any) => sum + Number(row[lastColumnKey]),
    0
  );
});
